...
editHistory:
  - date: '2026-02-07'
    changes: '收敛 Step6 的 ADR1-ADR7；统一 GraphStore 口径；补充迁移基准、权限映射与治理兜底'
  - date: '2026-01-31'
...
> 本项目的技术选型参考文档为"拓展信息/AI有机体构建平台 – 技术架构蓝图.md"，本参考文档不是最终确定版，仅供参考。
...
> **核心验证目标**：林远（独立创作者）在 11 分钟内完成以下 5 个时刻，首次体验到"AI 有状态"的核心价值。

| # | 时刻名称 | 体验描述 | 时间要求 | 验证指标 | 对应系统能力 |
|---|----------|----------|----------|----------|--------------|
| 1 | **注册 & 创建角色卡** | 选择「小说创作」模板，创建第一个角色卡"苏晓"（包含"怕水"设定） | ≤3 分钟 | 完成角色卡创建，系统自动保存 | Context Unit 创建 + Tag + Policy 基础配置 |
| 2 | **写章节自动识别关联** | 粘贴章节内容，系统自动识别"苏晓"并关联到角色卡 | ≤2 分钟 | 右侧面板亮起提示："检测到你提到了「苏晓」，已自动关联到她的角色卡。" | 自动引用识别 + Reference Edge 自动创建 |
| 3 | **冲突自动提示** | 系统标记"跳进河里救人"与"怕水"设定的冲突 | ≤3 分钟 | 提示："苏晓的设定中标注「怕水」。当前情节与设定可能存在冲突，是否需要添加剧情解释或更新设定？" | 冲突检测 + Activation Edge 触发传播 |
| 4 | **修改设定后影响提示** | 点击"更新设定"，系统立刻显示受影响的章节 | ≤1 分钟 | 提示："设定已更新。检测到以下章节引用了苏晓：第12章、第23章。其中涉及「怕水」描写的段落已标记，供您审查。" | 变更传播 + ImpactSet 计算 + 确认中心 |
| 5 | **确认并观察自动关联** | 接受建议，系统显示其他关联（如"青龙帮"势力卡） | ≤2 分钟 | 看到完整的自动关联链路，感知"系统在呼吸" | 传播引擎 + 可视化展示 + 查询 API |

**总时间要求**：**≤11 分钟**（3+2+3+1+2），预留 1 分钟给用户反应时间

**核心验证指标**：
- **首次"哇"时刻转化率**：用户在 11 分钟内完成上述 5 个时刻的比例 ≥80%
- **用户感知**：用户明确表示"它真的知道苏晓怕水？"或"系统在呼吸"

**对应场景驱动边界**：
- 林远的 P0 场景（必须完整支持）
- 对应 PRD 中的指标：首次"哇"时刻转化率 ≥80%、状态感知率 ≥80%
...
| **GraphStore** | IndraDB（默认实现）+ Ladybug（并行 PoC）；Neo4j 不进入 MAV/V1.5 主路径 | IndraDB 查询表达能力限制；Ladybug FFI 成本 | `GraphStore` trait 隔离；PoC Gate 决定 V1.5 默认（Ladybug 达标则切换，否则继续 IndraDB） |
...
| **GraphStore** | **IndraDB 默认实现**；Ladybug 并行 PoC；Neo4j 不进入 MAV/V1.5 主路径 | ⚠️ 暂定 | IndraDB 工程可控；Ladybug 能力对照评估；PoC Gate 决定 V1.5 默认 |
...
| 组件 | 决策 | 状态 | 迁移触发条件 |
|------|------|------|--------------|
| **EventStore** | 自研 ctxlog（借鉴 Memvid WAL/Checkpoint） | ⚠️ 暂定 | 事件量>100万 / 查询或重放延迟 P95>500ms@1万事件（连续7天，约1000 Units基准）/ 需真正单文件 / SQLite 瓶颈（profile 证据） |
| **协作同步** | Yrs（Yjs Rust port）+ libp2p | ⚠️ 暂定 | P2P 实时协作成为核心场景 |
| **图数据库** | PoC Gate 决定 V1.5 默认（Ladybug 达标则切换，否则继续 IndraDB） | ⚠️ 暂定 | 通过标准：四类边传播查询、影响范围导出、跨平台打包、License 合规 |
...
**决策门**：MAV 验证期结束前（3 个月内）完成 PoC；按 ADR-2 规则执行（Ladybug 达标则切换，否则继续 IndraDB）。
...
|------|---------|------|--------------|----------|
| **EventStore** | SQLite WAL | ⚠️ 暂定 | 事件量>100万 / 查询或重放延迟 P95>500ms@1万事件（连续7天，约1000 Units基准）/ 需真正单文件 / SQLite瓶颈（profile 证据） | 自研 ctxlog（借鉴Memvid） |
| **GraphStore** | IndraDB（默认实现）+ Ladybug（并行 PoC） | ⚠️ 暂定 | PoC 不达标 / FFI 成本不可接受 / 许可与供应链风险不可控 | 保持 IndraDB，并持续评估 Ladybug 或远期候选 |
...
1. **统一溯源作为唯一事实源**
平台将 Event Sourcing 作为唯一事实源，所有状态变化都以事件记录；图数据库仅作为可重建投影层。这使“AI 记住了什么、为何这么做”可追溯、可审计、可回放。

2. **有机体范式替代工作流范式**
用户不再编排“动作链路”，而是设计“存在系统”：Context Unit（器官）+ 四类边（神经/血管）+ 传播/确认机制（反射弧），让 AI 呈现持续状态与协同行为。

3. **治理内建而非后补**
Propagation Engine 与 Confirmation Center 构成治理闭环：自动发现影响、结构化给出 ChangeSet/ImpactSet/Reason、人类最终确认，确保“自动化”与“可控性”共存。

4. **模块化可替换底座**
通过 trait 隔离（EventStore/GraphStore/Provider），避免业务层绑定具体实现。张涛可以在不修改业务代码的情况下切换图数据库或迁移事件存储，只需重放事件流即可重建状态。

### Market Context & Competitive Landscape

| 维度 | 本项目 | 常见方案（Dify/LangChain 等） |
|------|--------|-----------------------------|
| 系统定位 | 设计 AI 的“存在”（有机体） | 编排 AI 的“动作”（流程） |
| 状态来源 | 事件溯源 + 投影重建 | 对话上下文 + 工作流状态 |
| 变更治理 | 传播引擎 + 确认中心 + 审计链 | 多为节点执行日志 |
| 多人协作可控性 | 权限、审批、审计一体化 | 常需外部拼装治理能力 |
| 架构演进性 | 适配层隔离、可替换组件 | 容易被具体中间件绑定 |
...
| 验证项 | 目标 |
|------|------|
| 查询/重放延迟（MAV） | P95 < 500ms |
| 单项目事件量（MAV 安全区） | ≤ 100 万 |
| 四类边传播查询（PoC Gate） | 正确率 100%，性能达标 |
| 快照→恢复一致性 | 100% 一致 |
| .ctxpkg 导入导出完整性 | 校验通过率 100% |
...
| 风险 | 影响 | 缓解策略 |
|------|------|---------|
| 事实源口径漂移 | “AI 记住了”失真 | 强制事件落盘；禁止业务层绕过 EventStore |
| 图数据库选型误判 | 研发返工/性能问题 | 双轨 PoC + 决策门 + 明确 fallback |
| 溯源粒度过重 | 成本与性能恶化 | 中粒度必填；完整推理链可选、可加密 |
| 子图归属不清 | 冲突归责与权限混乱 | `primary_graph` 唯一；跨图查询显式 scope |
| 协作自动化失控 | 信任崩塌 | 风险分级 + 审批链；高风险必须人工确认 |
| 跨子图越权查询 | 合规风险 | 五元 ACL + 查询审计事件强制记录 |

### Architecture Decision Notes

> 本组 ADR 的目标不是“技术选型罗列”，而是确保三层闭环稳定：  
> 事实层（ADR3+ADR5）→ 演化层（ADR1+ADR2+ADR4）→ 治理层（ADR6+ADR7）。

#### ADR-1: 分层图谱实现方案（阶段确认）

**MAV 决策：**
- 采用“单图 + 命名空间隔离”。
- 不做物理子图拆分，仅保留子图语义与投影接口。

**V1.5 触发条件（任一满足即升级投影子图）：**
- 单项目 Context Unit > 5000 且查询 P95 > 500ms（连续 7 天）。
- 跨命名空间查询占比 > 30%。
- 多角色明确提出“按责任子图视图”刚需。

#### ADR-2: 图数据库选型（阶段确认）

**MAV 决策：双轨 PoC，明确主次但不锁死。**

- **默认实现（Default）**：IndraDB（纯 Rust、嵌入式集成成本低、工程可控）。
- **并行评估（Evaluation）**：Ladybug（Cypher/检索能力对照评估）。
- **明确不做**：Neo4j 不进入 MAV/V1.5 主路径，仅作为远期服务端备选。

**PoC 决策门（MAV 结束前完成）：**

| 测试项 | 通过标准 |
|------|------|
| 四类边传播查询 | 正确率 100%，性能达标 |
| 影响范围子图导出 | 正确率 100%，性能达标 |
| 快照/恢复一致性 | 100% 一致 |
| 并发写入稳定性 | 无数据丢失、无不可恢复错误 |
| 跨平台 CI 打包 | Windows/macOS/Linux 全通过 |
| License 合规 | 分发策略可执行、无阻断风险 |

**决策规则：**
- 若 Ladybug 通过全部标准：V1.5 切换为默认。
- 若 Ladybug 未通过：继续使用 IndraDB，V1.5 重新评估候选。
...
- `trigger_event` `inputs_read` `conclusion`

**可选字段：**
- 完整推理链（CoT）、扩展证据链、候选方案比较
- 默认关闭；按 Tag/Policy 启用；支持加密存储

#### ADR-4: 子图划分策略（已确认）

**决策**：采用“三层混合”策略，并执行主属唯一约束。
...
| 对象 | primary_graph 规则 |
|------|--------------------|
| Unit | 创建时所属命名空间即 primary_graph |
| Event | 以 action 主体 Unit 的 primary_graph 为准 |
| Edge | 以源 Unit 的 primary_graph 为准 |

**影响域规则：**
- `affected_graphs` 可多值，用于记录跨图影响，不重复存储事实数据。

**跨图查询规则：**
1. 默认仅查询 `primary_graph`。  
2. 跨图必须显式声明 scope（禁止隐式全图扫描）。  
3. 需通过源图与目标图双侧权限检查，任一失败即拒绝。

**循环引用保护：**
- 设计时静态检查：构建依赖图时检测循环，给出重构建议。
- 运行时保护：传播执行遇到未检测到的循环 → 自动暂停 + 标记待处理。

#### ADR-5: 存储选型确认（阶段确认）

**MAV 决策：**
- 运行态：SQLite WAL（append-only 事件容器）。
- 同步态：`.ctxpkg`（唯一同步工件，checkpoint + backup 生成）。
- 约束：业务层禁止直接 SQL 读取业务状态。

**迁移到 ctxlog 触发条件（任一满足）：**
- 单项目事件量 > 100 万。
- 查询或重放延迟 **P95 > 500ms @ 1万事件**（连续 7 天）。
  - 测量基准：标准测试项目（约 1000 Units、1万 Events）。
  - 测量环境：中等配置设备（16GB RAM、SSD）。
- 需要“单文件解包即运行”能力。
- SQLite 成为明确性能瓶颈（profile 证据成立：CPU/IO 持续高负载）。

#### ADR-6: AI 维护权限模型（阶段确认）

**MAV 决策**：风险分级 + 审批链（Trust Score 延后到 V1.5）。

**风险分级与确认模式映射：**

| 风险等级 | 对应确认模式 | 行为 |
|---------|-------------|------|
| P0 Critical | strict | 必须人工逐条确认，不允许自动执行 |
| P1 High | confirm_required | Maintainer 提交，Authority Source 审批 |
| P2 Medium | batch_queue | 可批量处理，但需人工触发确认 |
| P3 Low | yolo | 可自动执行，必须可回滚且审计落盘 |

**映射约束：**
- 可“收紧”不可“放宽”：允许将低风险提升为更严格模式，不允许将 P0/P1 降为 yolo。
- 高风险变更（删除关系、修改核心设定）必须人工确认，不得绕过确认中心。

**V1.5 增强：**
- 引入 Trust Score 动态授权（基于历史准确率、采纳稳定性进行自动提权/降权）。

**审批链兜底（MAV基础版）：**
- 审批任务创建后超时（默认 7 天，可配置）无响应 → 自动转交上级 Authority Source。
- 无上级可转交 → 进入系统待处理队列，暂停相关传播。
- 对 P0/P1 高风险变更，禁止“超时自动通过”。

#### ADR-7: 权限细节与跨子图查询（已确认）

**决策：五元 ACL。**
- `namespace + role + action + scope + policy_version`
...
| 迁移到 Neo4j（远期服务端备选）成本高 | 早期设计兼容性层，避免深度特性绑定 | 抽象层覆盖率 |
